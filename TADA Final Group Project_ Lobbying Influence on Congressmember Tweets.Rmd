...
phrases <- phrase(c("gun violence", "gun safety", "common sense", "prevent gun*","commonsense gun", "senseless gun*", "gun lobby", "gun storage", "gunviolence prevention", "unsecured gun*", "gun*"))
...
guns_dic1 <- dictionary(list(guns = phrase(c("gun violence", "gun*safety", "common*sense", "prevent gun*","commonsense*gun", "senseless*gun*", "gun*lobby", "gun*storage", "gunviolence*prevention", "unsecured*gun*", "gun*"))))
...
```{r}
envi_dic <- dictionary(list(envi = phrase(c('climate*','environmental*protection', 'environmental*resources', 'the*arctic', 'natural*resources', 'interior', 'EPA', 'climate*', 'natural*disaster', 'natural*gas'))))
                                     
#check party dfm against new dictionary
dfm_envi_dic <- dfm(dfm_new, dictionary = envi_dic, group = "Party")


dfm_individ <- dfm(dfm_new, group = "screen_name", dictionary = envi_dic)
...
#merge both of these files to have gun and env counts

TWITTER_ANALYSIS <- merge(x = individ_guns_final, y = individ_env_final, by = "congress_id_handles.TweetCongress", all = FALSE)


#merge twitter counts with our lobby amount totals, based on common congressmen ID (twitter handle)

FINAL_GUN_ANALYSIS <- merge(x = TWITTER_ANALYSIS, y = df_money_gun_peryear, by = "congress_id_handles.TweetCongress", all = FALSE)

FINAL_ENV_ANALYSIS <- merge(x = TWITTER_ANALYSIS, y = df_money_env_peryear, by = "congress_id_handles.TweetCongress", all = FALSE)

#Calculating basic regression model to test correlation of our hypothesis 
#GUN
reg_basic <- lm(guns ~ Total + report_year.x + congress_id_handles.TweetCongress, FINAL_GUN_ANALYSIS)
stargazer(reg_basic, type ="html")

#ENVIRONMENT
reg_basic2 <- lm(envi ~ Total + issue_code + report_year.x + congress_id_handles.TweetCongress, FINAL_ENV_ANALYSIS)
